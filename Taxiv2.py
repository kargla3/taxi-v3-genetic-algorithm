...
class TaxiGeneticAlgorithm:
    ...
    def get_best_move(
        self, taxi_pos: Tuple[int, int], target_pos: Tuple[int, int]
    ) -> int:
    ...
    def create_heuristic_individual(
        self, individual_id: int, intelligence_level: float = 0.9
    ) -> Individual:
        """Tworzy osobnika z heurystyką opartą na logice"""
        genotype = []

        for state in range(self.genotype_size):
            taxi_row, taxi_col, passenger_loc, destination = self.env.unwrapped.decode(
                state
            )
    ...
    def initialize_population(self):
        ...
        for i in range(
            very_smart_count + smart_count,
            very_smart_count + smart_count + weak_smart_count,
        ):
        ...
        for i in range(
            very_smart_count + smart_count + weak_smart_count, self.population_size
        ):
        ...
        print(
            f"- Losowych: {self.population_size - very_smart_count - smart_count - weak_smart_count}"
        )
    ...
    def evaluate_individual(self, individual: Individual, episodes: int = 20) -> float:
            ...
            while not done and step < max_steps:
                ...
                if reward == 20:  # Sukces - dotarcie do celu
                    ...
                    if action == 4:  # Próba pickup
                        taxi_row, taxi_col, passenger_loc, _ = (
                            self.env.unwrapped.decode(state)
                        )
                        if passenger_loc < 4:
                            passenger_pos = self.locations[passenger_loc]
        ...
        return max(
            fitness, avg_reward
        )  # Fitness nie może być gorszy niż średnia nagroda
    ...
    def smart_mutate(self, individual: Individual):
        ...
        adaptive_rate = self.mutation_rate * (
            1.0 - self.generation / self.num_generations
        )
            ...
            if self.rng.random() < adaptive_rate:
                ...
                if passenger_loc < 4:  # Pasażer czeka
                    ...
                    if taxi_pos == passenger_pos:
                        ...
                        new_action = self.rng.choice(
                            [4, 0, 1, 2, 3], p=[0.7, 0.075, 0.075, 0.075, 0.075]
                        )
                    else:
                    ...
                    if taxi_pos == destination_pos:
                        ...
                        new_action = self.rng.choice(
                            [5, 0, 1, 2, 3], p=[0.7, 0.075, 0.075, 0.075, 0.075]
                        )
                    else:
    ...
    def print_generation_stats(self):
        ...
        success_rates = [
            ind.success_rate for ind in self.population if hasattr(ind, "success_rate")
        ]
        ...
        if success_rates:
            ...
            print(
                f"Osobników z sukcesem: {successful_individuals}/{len(success_rates)}"
            )
            ...
            "osobnicy": [
                {
                    "id": int(ind.id),
                    "genotyp": ", ".join(
                        symbol_map[g] for g in ind.genotype if g in symbol_map
                    ),
                    "fitness": float(ind.fitness),
                    "success_rate": float(ind.success_rate),  # Dodano wskaźnik sukcesu
                }
